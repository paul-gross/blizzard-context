# Verifiability matrix — blizzard (`bzh:verifiability-matrix`)

An inventory of the verification methods for the blizzard ecosystem — each entry one way a skill or agent may assert a
blizzard change is correct. Conforms to the canon concept at `winter-canon:/verifiability-matrix.md`.

Method ids follow the canon's scheme at `winter-canon:/verifiability-matrix.md#method-identifiers`: commands and manual
methods are `<scope>:<method>` (a manual method's method name is `manual`); tools are unscoped under a flat `tool:`.
Scopes here: **`blizzard`** for the app repo's Python QA and the daemon-level tiers, **`web`** for the Angular workspace
checks, **`blizzard-mock`** for the mock-fleet repo.

Full detail lives under [./blizzard/](./blizzard/), one file per reader question. Each is a single owner; a fact stated
in one is linked from the others, never restated.

## Routing

### The inventory, row by row

| File                                                          | Read when…                                                                                         |
| ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| [`commands.md`](./blizzard/commands.md)                       | …you need what a Commands row runs, asserts, and cannot see — every row marked *(more)*.           |
| [`e2e-scenarios.md`](./blizzard/e2e-scenarios.md)             | …you are adding, renaming, or reading a `blizzard:e2e` scenario — what each one proves.            |
| [`manual.md`](./blizzard/manual.md)                           | …you are performing a `blizzard:` manual method: its surface, setup, steps, and pass condition.    |
| [`manual-mock.md`](./blizzard/manual-mock.md)                 | …you are performing a `blizzard-mock:` manual method — the live forge, the seeded board and fleet. |
| [`tools.md`](./blizzard/tools.md)                             | …you are standing up the state a verification needs — every Tools row marked *(more)*.             |
| [`markers.md`](./blizzard/markers.md)                         | …a row carries a `P3`–`P7` or **Gap** marker, or its method does not exist yet.                    |
| [`acceptance-criteria.md`](./blizzard/acceptance-criteria.md) | …you need which method proves which MVP acceptance criterion.                                      |

### The standard a test is held to

| File                                                      | Read when…                                                                                             |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| [`tier-rules.md`](./blizzard/tier-rules.md)               | …you need the tier roster, or are writing a test at one — hermeticity, mocks, viewport, log sinks.     |
| [`companion-changes.md`](./blizzard/companion-changes.md) | …your change touches a `tests/e2e/` case or a hub↔runner wire surface — each owes a companion landing. |
| [`evidence.md`](./blizzard/evidence.md)                   | …you are judging whether a green run actually pins the behavior its name claims.                       |
| [`pre-push.md`](./blizzard/pre-push.md)                   | …you are about to push — the sweeps that stand in for what a local gate cannot reach.                  |
| [`gaps.md`](./blizzard/gaps.md)                           | …you are tempted to add a tier for something no tier covers — what stands in for one, and why.         |

## Commands

Verification that runs as a single command — exit 0 is the pass signal.

| Method                     | Command                                                                                                     |
| -------------------------- | ----------------------------------------------------------------------------------------------------------- |
| blizzard:build             | `uv sync` from the repo root — the `blizzard` project and its `dev` group.                                  |
| blizzard:lint              | `uv run ruff check .` ([python](../standards/python.md)).                                                   |
| blizzard:format            | `uv run ruff format --check .` ([python](../standards/python.md)).                                          |
| blizzard:typecheck         | `uv run pyright` ([python](../standards/python.md)).                                                        |
| blizzard:unit-test         | `uv run pytest -m unit` — one class or function in isolation. *(more)*                                      |
| blizzard:component-test    | `uv run pytest -m component` — a domain slice, doubles only at the seams. *(more)*                          |
| blizzard:sse-contract      | `mise run sse-contract` — the SSE frame shape against the golden corpus `contracts/sse/`. *(more)*          |
| blizzard:prose-ratchet     | `mise run prose-check` — the per-root prose ratchet ([prose budget](../standards/prose-budget.md)).         |
| blizzard:restatement-sweep | `mise run restatement-check` — the one-home census ([one home](../standards/one-prose-home.md)). *(more)*   |
| blizzard:gate              | `mise run gate` — the local reproduction of CI's shared `gate` job. *(more)*                                |
| blizzard:wheel             | `mise run build` — both Angular apps, then the one wheel, node-free. *(more)*                               |
| blizzard:wheel-smoke       | The serve smoke on the built wheel in a node-free venv — the **P5 exit criterion**. *(more)*                |
| blizzard:image-smoke       | `mise run image-smoke` — the hub image booted on an empty data volume. *(more)*                             |
| blizzard:compose-smoke     | `mise run compose-smoke` — the reference compose deployment on a local image. *(more)*                      |
| blizzard:ci                | `gh run watch --repo paul-gross/blizzard <run-id> --exit-status` — the authoritative remote gate. *(more)*  |
| blizzard:e2e               | `mise run e2e` — the standing full-stack smoke suite ([registry](./blizzard/e2e-scenarios.md)).             |
| blizzard:service-test      | `mise run service-test` — a running daemon's HTTP API driven from outside. *(more)*                         |
| blizzard:crash-sweep       | `mise run crash-sweep` — the full kill-9 sweep over the crash-point registry. *(more)*                      |
| blizzard:journey           | `mise run journey` — the capstone acceptance-journey rehearsal over real host daemons. *(more)*             |
| web:lint                   | `npm run lint` in `web/` — eslint over the Angular workspace ([frontend](../standards/frontend.md)).        |
| web:unit-test              | `npm run test` in `web/` — vitest, the frontend unit/component tier ([frontend](../standards/frontend.md)). |
| web:typecheck              | `npm run build` in `web/` — a real AOT compile of both Angular apps. *(more)*                               |
| web:client-drift           | `npm run generate:client` in `web/`, then fail on any unstaged diff (`bzh:generated-client`). *(more)*      |
| web:structural-gate        | `npm run structural-gate` in `web/` — the `max-lines` ceiling and the real-timer sweep. *(more)*            |
| web:shell-sweep            | `npm run shell-sweep` in `web/` — the real-Chromium proof for what jsdom cannot evaluate. *(more)*          |
| blizzard-mock:build        | `uv sync` in the `blizzard-mock` repo.                                                                      |
| blizzard-mock:lint         | `uv run ruff check .` ([python](../standards/python.md)).                                                   |
| blizzard-mock:format       | `uv run ruff format --check .` ([python](../standards/python.md)).                                          |
| blizzard-mock:typecheck    | `uv run pyright` ([python](../standards/python.md)).                                                        |
| blizzard-mock:unit-test    | `uv run pytest` — the mock fleet's own unit + component suite, plus the wire-parity guard. *(more)*         |
| blizzard-mock:e2e          | `uv run pytest -m e2e` — the fleet acceptance proof, and the **P4 exit criterion**. *(more)*                |

## Manual testing

Verification no single command performs. Full per-method surface, setup, steps, and pass condition:
[./blizzard/manual.md](./blizzard/manual.md) for the `blizzard:` rows,
[./blizzard/manual-mock.md](./blizzard/manual-mock.md) for the `blizzard-mock:` rows.

| Method                               | Exercises                                                                                              |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------ |
| blizzard:manual                      | The acceptance loop end-to-end. **Automated as of P6** — run it as `blizzard:e2e` rather than by hand. |
| blizzard:manual-sse-probe            | The live SSE wire probe — framing and timing on a real socket, hub or runner, one daemon at a time.    |
| blizzard:manual-standing-idp         | Auth-gated behavior in a browser against a standing hub and stub IdP, outside any test fixture.        |
| blizzard:manual-external-usage-probe | The vendor's real OAuth-usage response shape, proven live against `claude`'s own `/usage`.             |
| blizzard:manual-autocompact-window   | A declared `--autocompact` window compacting a real session, rather than the model's own maximum.      |
| blizzard:manual-rollback-drill       | The compose deployment's rollback promise, walked for real against two published image tags.           |
| blizzard-mock:manual                 | The winter-wired mock forge fronting a real fixture workspace's bare origins.                          |
| blizzard-mock:manual-seeded-board    | A realistic board rendered from a direct store seed — no work source configured, no hub restart.       |
| blizzard-mock:manual-seeded-fleet    | A seeded runner panel beside a seeded board, still coherent after the daemon's first reconciling tick. |

## Tools

Setup an agent uses to stand up the scenario a verification needs — not assertions of correctness themselves.

Full per-tool detail for the rows marked *(more)*: [./blizzard/tools.md](./blizzard/tools.md).

| Tool                   | Use                                                                                                                    |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| tool:service-up        | `winter service up <env> --wait` — the verification stack for a feature env, port-band isolated. *(more)*              |
| tool:mock-fleet        | The `blizzard-mock` fleet — forge, fixture workspace, mock harness, mock hub/runner, stub IdP. *(more)*                |
| tool:mock-data         | The mock-data CLI (`blizzard-mock-data`) — seeding a hub or runner store into a known world. *(more)*                  |
| tool:fixture-workspace | The fixture-workspace scaffold (`blizzard-mock-fixture`) — bare `file://` origins plus a throwaway workspace. *(more)* |

## See also

- [../architecture/crash-correctness.md](../architecture/crash-correctness.md) — the daemon requirements the
  `blizzard:crash-sweep` method exercises.
