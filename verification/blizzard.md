...
This is a deliberately-built method closing a real, permanent gap, not a placeholder for a future CI tier — the same shape as `blizzard:manual-sse-probe` and `blizzard:manual-rollback-drill` above: no tier will ever be added to replace it, because the thing it proves (an external vendor's live, undocumented response shape) is structurally outside what a hermetic, network-free CI tier can ever see.
