...
| web:structural-gate        | `npm run structural-gate` in `web/` — a `max-lines` ceiling, a retired-chrome-class grep sweep (`bzh:frontend-kit`), an empty-state-without-the-kit sweep (`bzh:frontend-empty-state-gated`), and a real-timer sweep failing a literal non-zero `setTimeout`/`setInterval` delay in a merge-gating spec. *(more)*                                                                                                                                                                                                                                |
...
Setup: a freshly provisioned, not-yet-seeded env, its shell vars sourced (see
[../tooling/store-seeding.md](../tooling/store-seeding.md)'s §Running it for why a plain shell needs
`source <(winter env <env>)` before `$BZ_HUB_RUNTIME`/`$BZ_HUB_WEB_PORT` resolve):
...
| tool:mock-data         | Seeding a hub or runner store into a known world — the mock-data CLI (`blizzard-mock-data`), from a clean reset through one hand-placed concept to a whole ready-to-view board. *(more)* |
