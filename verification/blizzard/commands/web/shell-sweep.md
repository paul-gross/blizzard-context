...
- `gardening-findings-triage.shell-sweep.spec.ts` covers the findings triage list (`FleetFindingList`, blizzard#401
...
- `gardening-finding-triage-dialog.shell-sweep.spec.ts` covers the findings triage bulk-action dialog's own view
  (blizzard#401 Phase 4), mounted directly with plain inputs, once per verb that carries a distinct field shape. At
