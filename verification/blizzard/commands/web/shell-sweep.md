...
- `finding-fact-timeline.shell-sweep.spec.ts` covers the finding detail panel's fact-chain timeline, mounted through the
  ...
  unbroken long note (an ordinary-prose fixture wraps at word boundaries regardless of `overflow-wrap: anywhere` and
...
- `chunk-transcripts-provenance.shell-sweep.spec.ts` covers the transcripts tab's per-segment harness-provenance
  badges (blizzard#441): two segments recording distinct harnesses render two genuinely distinct badges, at the
  board's own narrow, mobile-reachable width, with no page error and no horizontal overflow.
...
- `chunk-detail-view-provenance.shell-sweep.spec.ts` covers the runner's local-panel escalation resume box's
  harness-provenance badge (blizzard#441): it renders beside the resume command at the runner's own narrow width,
