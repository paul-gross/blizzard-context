...
member — lands `blizzard`'s `tests/service/support.py::_fake_binary` kwarg and name-mapping in the same change, kept
1:1 with the enum, and updates `_fake_binary`'s `len(lever_flags)` pin to match.

**Why.** `blizzard` deliberately holds no dependency on `blizzard-mock` (`tests/support.py::github_double`'s
established stance), so nothing mechanical diffs the two rosters — a member landed on one side alone drifts silently,
covered only by [`./gaps.md`](./gaps.md#the-cross-repo-opencode-lever-roster).
