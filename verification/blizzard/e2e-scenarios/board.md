...
- `test_board_browser_live_group_reorder_answer_and_pause` — loads the board once, never reloading, proving the status
  ...
  While the dock is open on that chunk it also asserts none of the three retired dependency-UI testids (the prerequisite
  field, Declare, Release) render. And it proves the runner registry's pause/resume brake stops and then restarts new
  claims (MVP criterion 11), engaged before the survivor lands because the landing frees the runner's only agent slot in
  the same tick — FILL's reconcile releases the survivor's binding — so from that instant the brake alone holds the
  remaining ready chunk in the lane.
