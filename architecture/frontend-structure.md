...
in the SSE dispatch registry (`sse/fleet-live.ts`) rather than a new `case` in a hand-written switch. What a sub-barrel
...
export line plus one table row" — additive, not contended.

**Detect.** A new top-level export added directly to `public-api.ts` instead of a feature sub-barrel; a new `case` added
to `fleet-live.ts`'s `dispatch()` instead of a registry row; a sub-barrel export no consumer outside its own feature
directory imports.
...
`ChunkDetailHeader` — stays unexported. A new live-updated feature adds a row keyed by its event type to
...
event type is a compile error) rather than a `case` in `dispatch()`.
