...
**Scope.** Governs authored graph YAML and its prompts. The **fleet protocol** is not application knowledge and stays in the graph: `blizzard runner artifact create --name <n>` (an asset), `blizzard runner artifact commit` (a git commit), `blizzard runner ask`, and the work-item proxy (`blizzard runner work-items`) are blizzard's own surface, identical across every application it drives. `blizzard runner attach` is a deprecated alias for `artifact create` ([../standards/worker-nodes.md](../standards/worker-nodes.md) `bzh:worker-node-attach-instruction`) — no packaged prompt names it.
