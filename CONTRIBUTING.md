...
- Give every rule its stable id, per the scheme `winter-canon:/rule-shape.md` owns — with two differences here: the
  ...
  and the rule's heading is the id's only home — this harness keeps no id registry to re-sync, so an id resolves by
  grepping the tree for its slug, a deliberate trade while the rule count stays greppable and revisited if resolution
  ever misses. Adding or renaming an id updates every citation in the same change.
...
- `blizzard-context:markdown-format`, `blizzard-context:markdown-lint`, and `blizzard-context:markdown-prose-lint` —
