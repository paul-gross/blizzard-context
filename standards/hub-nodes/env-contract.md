...
| `BZ_HUB_REVIEW_FINDINGS_URL`                       | Submits a review round's deferred findings via a bodyless `POST` — the route reads the chunk's own newest `review-finding-delta` artifact — read only by `review_deliver.py`, present under the same condition as `BZ_HUB_GARDEN_DELIVERY_URL` |
