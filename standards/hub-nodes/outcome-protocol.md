...
- `land_pr_ci.py` prints `pending` while any repo's PR is not yet eligible to merge — a `clean`/`blocked`/`unstable` PR
  whose head has no terminal check-run failure but is not yet all-green either, or any other unsettled state — and no
