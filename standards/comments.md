...
- An unresolvable reference in a generated description — `blizzard/tests/test_openapi_descriptions.py` fails the unit tier on the three `Goes` shapes above, so this half is enforced rather than remembered. It scans the committed specs **and** the `wire/` models no spec reaches, which are one `responses=` away from being public and are held to the same three shapes.
