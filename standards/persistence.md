...
**Detect.** A migration importing a table from `blizzard.{hub,runner}.store.schema` that a later revision in the same tree reshapes; a frozen `sa.Table` literal whose constraint set — especially a foreign key — is narrower than `schema.py` declared for that table at this revision's point in history, for a table this revision creates; a `NoReferencedTableError` resolved by deleting the offending `sa.ForeignKey` instead of adding a resolution stub.

**Do.** `blizzard/src/blizzard/hub/store/migrations/versions/20260714_0819_delivery_pr_facts.py`'s `_frozen_metadata` block: a local `MetaData()`, a `chunks` stub table carrying just the referenced column, and the frozen `delivery_pr_opened` table whose `sa.ForeignKey("chunks.chunk_id")` resolves against the stub. `20260713_1218_walking_skeleton_facts.py` is the original precedent for the same idiom, applied there to every table it creates. For the read-only-stub case, `blizzard/src/blizzard/hub/store/migrations/versions/20260715_1817_chunk_promoted.py`'s `chunks` table carries only `chunk_id` and `minted_at` — the two columns its back-fill `select`s.
