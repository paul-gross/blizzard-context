...
| [./worker-nodes.md](./worker-nodes.md) | Authoring or reviewing a worker node that declares a `produces:` entry — the prompt's declaration instruction, the judgement-assessment fallback, the `produces_mode` enforcement backstop, and the worker's injected identity env |
