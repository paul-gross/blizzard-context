...
**Detect.** A raw `rgba(0, 0, 0` in a component `.css` — a review question, not tooled by `web:lint`
([../verification/blizzard.md](../verification/blizzard.md)). The retired-chrome-class check (`bzh:frontend-kit-floor`'s
Detect names the classes) is tooled instead, by `web:structural-gate`'s kit-floor sweep.
