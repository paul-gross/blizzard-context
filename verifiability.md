...
| `blizzard-context:lint-script-tests`    | `python3 tests/test_lint_markdown_style.py` — exercises the contributed `winter lint` check (`scripts/lint-markdown-style.py`, wired via `winter-ext.toml`'s `lint` field) against stdlib-only fixtures with stubbed tool binaries, needing neither tool installed. Mirrors `winter-workflow:lint-tests`.                                                               |
