...
- The escalation **carries both the wrapped takeover verb and the literal session-resume command beside it**: the wrapped `blizzard runner takeover` form is the supported entry point — it records the takeover fact with the daemon first, so no loop step can respawn or judge the held session while a person holds it — and the literal raw resume command rides alongside as the fallback for a host without the wrapped verb, or an escalation recorded by an older runner. Either way the person starts where the worker stopped — and beside it, the parked session's own identity: which declared session it belonged to, and the model and effort it actually ran under. Those are read back from what the session ran, never re-derived, so the operator lands in the configuration the fleet was using rather than whichever one a fresh resolution would produce now. A session that predates the record simply carries none of them, and the command stays bare rather than guessing.
