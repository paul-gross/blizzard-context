...
- The escalation **carries a raw resume command and, only sometimes, a wrapped takeover verb beside it — but neither is guaranteed present** — the two are related one way, not symmetrically: a present wrapped verb always means the raw command is present too, but a present raw command does not mean the wrapped verb is, and either or both can be empty. The wrapped verb is present only when every one of these holds: the escalation was composed by a **runner**, never the hub itself; that runner held a resumable session for the parked lease; bindings were present; and the runner's own runtime location was configured. It is missing, and the raw command varies with it, in four ways: a **hub-authored cross-graph-unresolvable escalation** carries no wrapped verb (the hub has no runner runtime to compose it from) and its raw field holds hub-authored guidance rather than a runnable command — it tells the operator which graph to mint before requeuing, not how to resume a session; a **hub-authored bounce-cap escalation** likewise carries no wrapped verb, and its raw field is empty too — there is no command of any kind to run, only the escalation itself as the signal; a **runner-composed escalation whose runtime location wasn't configured** composes the raw command normally but leaves the wrapped one empty, with nowhere for it to point; and with **no parked session at all** — whether hub-authored or a runner escalating before any session existed — both fields are empty together. Either way a present wrapped verb is the supported entry point, entering under the daemon-first recording guarantee that keeps the loop off a held session while a person holds it (§Takeover below) — and beside either command, the parked session's own identity: which declared session it belonged to, and the model and effort it actually ran under. Those are read back from what the session ran, never re-derived, so the operator lands in the configuration the fleet was using rather than whichever one a fresh resolution would produce now. A session that predates the record simply carries none of them, and the command stays bare rather than guessing.
