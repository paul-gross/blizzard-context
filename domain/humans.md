...
- The escalation **carries the raw resume command, and only sometimes the wrapped takeover verb beside it** — the two are related one way, not symmetrically: a present wrapped verb always means the raw command is present too, but a present raw command does not mean the wrapped verb is. The wrapped verb is present only when every one of these holds: the escalation was composed by a **runner**, never the hub itself; that runner held a resumable session for the parked lease; bindings were present; and the runner's own runtime location was configured. It is missing three ways: a **hub-authored escalation** — a bounce-cap or a cross-graph-unresolvable one, both authored by the hub itself and never a runner — never carries the wrapped verb, since the hub has no runner runtime to compose it from, and its raw field may hold hub-authored guidance rather than a runnable command at all (an unresolvable migration's, for instance, tells the operator which graph to mint before requeuing, not how to resume a session); a **runner-composed escalation whose runtime location wasn't configured** composes the raw command normally but leaves the wrapped one empty, with nowhere for it to point; and with **no parked session at all**, both fields are empty together — the one case where they actually move in lockstep. Either way a present wrapped verb is the supported entry point, entering under the daemon-first recording guarantee that keeps the loop off a held session while a person holds it (§Takeover below) — and beside either command, the parked session's own identity: which declared session it belonged to, and the model and effort it actually ran under. Those are read back from what the session ran, never re-derived, so the operator lands in the configuration the fleet was using rather than whichever one a fresh resolution would produce now. A session that predates the record simply carries none of them, and the command stays bare rather than guessing.
