...
- It **closes by supersession, never resolution** — there is no "resolved" fact to write. Two things supersede one. Requeueing makes the chunk leasable again, and the next attempt's lease closes it. **Stopping the chunk** also closes it: a human who finished the work outside the fleet and then abandoned the chunk has resolved the hold, and no lease mint will ever follow a stop, so without this arm the escalation would stand forever. A runner that raised the escalation learns the stop on its next reconciliation and mirrors it locally, so its own list and panel drop the chunk too — meaning the runner-side close lags the hub's by a tick, and by a redeploy on a runner too old to sweep.
