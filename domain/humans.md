...
- **A present wrapped takeover verb always means the raw resume command is present too, but not the other way around** — an escalation can carry the raw command without the wrapped verb, and either or both can be missing. The wrapped verb is present only when the escalation was composed by a **runner** that held a resumable session for the parked lease, had already committed to the environment(s) it was working in, and knew its own location to point the command back at.
...
- **There are two genuinely distinct reasons the wrapped verb can be missing:**
